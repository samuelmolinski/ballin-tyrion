...
class EC_Calendar {
    ...
    */
	...
	function displayWidget($year, $month, $days = array(), $day_name_length = 2) {
		...
		$m = ucfirst($this->locale->get_month($month));
		...
		if ($day_name_length) {
			...
			$calendar .= '<thead style="position:relative;"><tr>' . "\n";
			
			foreach($day_names as $k=>$d) {
				if(0 == $k){
					$calendar .= '<th abbr="' . $d . '" scope="col" title="' . $d .'"><span class="cap capleft"></span>' 
								 . ($day_name_length < 4 ? $this->utf8_substr($d,0,$day_name_length) : $d)
								 . '</th>' . "\n";
				} elseif (6 == $k) {
					$calendar .= '<th abbr="' . $d . '" scope="col" title="' . $d .'"><span class="cap capright"></span>' 
								 . ($day_name_length < 4 ? $this->utf8_substr($d,0,$day_name_length) : $d)
								 . '</th>' . "\n";
				} else {
		...
		for ($day=1, $days_in_month = gmdate('t', $first_of_month); $day <= $days_in_month; $day++, $weekday++) {
			...
			$dayID = ' class="calendar-day"';

			// today
			$theday = mktime(0,0,0,$month,$day,$year);
			if ($theday == $today)
				$dayID = $adaptedCSS ? ' id="today"' : ' id="todayWidget" class="calendar-day" style="' . $todayCSS . '"';
