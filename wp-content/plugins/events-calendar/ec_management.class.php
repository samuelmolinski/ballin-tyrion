	...
	function display() {
		...
		if(isset($_POST['EC_addEventFormSubmitted'])) {
			...
			/*if(!empty($endTime) && !empty($startTime) || !is_null($endTime) && !is_null($startTime)&&($startDate != $endDate)) {
				$output .= '<div class="wrapper-date"><div class="bars w30"></div><div class="date w70">'.$startDate.' '.$startTime.' - '.$endDate.' '.$endTime.'</div></div>';
			} elseif(!empty($endTime) && !empty($startTime) || !is_null($endTime) && !is_null($startTime)&&($startDate == $endDate)) {
				$output .= '<div class="wrapper-date"><div class="bars w40"></div><div class="date w60">'.$startDate.' '.$startTime.' - '.$endTime.'</div></div>';
			} elseif((!empty($startTime) || !is_null($startTime))&&($startDate != $endDate)) {
				$output .= '<div class="wrapper-date"><div class="bars w60"></div><div class="date w40">'.$startDate.' '.$startTime.' - '.$endDate.'</div></div>';
			} elseif($startDate != $endDate) {
				$output .= '<div class="wrapper-date"><div class="bars w70"></div><div class="date w30">'.$startDate.' - '.$endDate.'</div></div>';
			} elseif($startDate == $endDate) {
			...
			if((!empty($endTime) && !empty($startTime) || !is_null($endTime) && !is_null($startTime))&&($startDate != $endDate)) {
				...
				$output .= "<strong>"._c('Time','events-calendar').": </strong>$startDate - $endDate<br />";
			} elseif(!empty($endTime) && !empty($startTime) || !is_null($endTime) && !is_null($startTime)) {
				$output .= "<strong>"._c('Time','events-calendar').": </strong>$startTime - $endTime<br />";
			...
			$splitDate2 = split("-", $endDate);
			if(4 == strlen($splitDate[0])){
				...
				$endDate = $splitDate2[2].'-'.$splitDate2[1].'-'.$splitDate2[0];
			}
		...
		if(isset($_POST['EC_editEventFormSubmitted'])) {
			...
			if(4 == strlen($splitDate[0])){
				...
				$this->month = $splitDate[1];
				$this->year = $splitDate[2];
				// inverse if international date format
				$startDate = $splitDate[2].'-'.$splitDate[1].'-'.$splitDate[0];
				$endDate = $splitDate2[2].'-'.$splitDate2[1].'-'.$splitDate2[0];
			}
    ...
    <div id="EC_alertmsg" class="alertmsg">
      ...
      <table id="EC_management-add-form" summary="Event Add Form" class="ec-edit-form">
        <tr>
          ...
          <td><input class="ec-edit-form-text" type="text" name="EC_title" id="EC_title"  maxlength=55/></td>
        </tr>
        <tr>
        ...
        <tr>
          ...
          <td><textarea class="ec-edit-form-textarea" name="EC_description" id="EC_description" maxlength=200></textarea></td>
        </tr>
        <tr>
...
?>
    ...
    <div id="EC_alertmsg" class="alertmsg">
      ...
      <table id="EC_management-edit-form" summary="Event Edit Form" class="ec-edit-form">
        <tr>
          <th scope="row"><label for="title"><?php _e('Title','events-calendar'); ?></label></th>
          <td><input class="ec-edit-form-text" type="text" name="EC_title" id="EC_title" value="<?php echo htmlentities(stripslashes($event->eventTitle),ENT_QUOTES);?>" maxlength=55/></td>
        </tr>
        <tr>
          <th scope="row"><label for="location"><?php _e('Location','events-calendar'); ?></label></th>
...
?>
    ...
    <form name="optionsEventsCalendar" method="post" action="?page=events-calendar-options" onLoad="">
      ...
      <p class="submit">
		...
		<input type="hidden" name="business" value="me@lukehowell.com">
		<input type="hidden" name="item_name" value="Luke Howell">
		<input type="hidden" name="no_shipping" value="1">
		<input type="hidden" name="return" value="http://www.lukehowell.com/thanks-for-the-donation/">
		<input type="hidden" name="no_note" value="1">
		<input type="hidden" name="currency_code" value="USD">
		<input type="hidden" name="tax" value="0">
		<input type="hidden" name="lc" value="US">
		<input type="hidden" name="bn" value="PP-DonationsBF">
		<input border="0" type="image" src="https://www.paypal.com/en_US/i/btn/btn_donate_LG.gif" name="submit" alt="Donate Now">
		<img height="1" border="0" width="1" alt="" src="https://www.paypal.com/en_US/i/scr/pixel.gif">
		</form>
    <script type="text/javascript">
    // <![CDATA[
      jQuery.noConflict();
